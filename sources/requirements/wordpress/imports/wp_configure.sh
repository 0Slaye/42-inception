...
USER_EXISTS=$(su -s /bin/bash www-data -c "wp-cli.phar user list --field=user_login --search=${WP_USER} --path=$WP_PATH")
if [ ! "$WP_USER" == "$USER_EXISTS" ]; then
	echo "Creating second user..."
	su -s /bin/bash www-data -c "wp-cli.phar --path=${WP_PATH} user create ${WP_USER} ${WP_USER_EMAIL} --role=subscriber --user_pass=${WP_USER_PASSWORD}"
else
	echo "Second user already created."
fi
exec php-fpm7.4 -F -R